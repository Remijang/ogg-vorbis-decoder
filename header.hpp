    ...
    void header_decode(io_buf &in, identification &id, int fl, int re) {
        ...
        if (flag == 1) {
            ...
            for (int i = 0; i < coupling_step; ++i) {
                ...
                if (angle[i] == magnitude[i] || magnitude[i] > id.audio_channels - 1 || angle[i] > id.audio_channels - 1)
                    exit(-1);
            }
        } else
            coupling_step = 0;
        ...
        for (int i = 0; i < submap; ++i) {
            ...
            if (submap_floor[i] >= fl)
                exit(-1);
            submap_residue[i] = in.read_u(8);
            if (submap_residue[i] >= re)
                exit(-1);
        }
        return;
    }
...
struct modes {
    ...
    void header_decode(io_buf &in, int ma) {
        ...
        mapping = in.read_u(8);
        if (windowtype != 0 || transformtype != 0 || mapping > ma)
            exit(-1);
        return;
    }
};
